...
Сравнение KEDA с классической связкой HPA + prometheus-adapter:

| Возможность | KEDA | HPA + prometheus-adapter |
|---|---|---|
| Скейлинг до нуля реплик | ✅ `minReplicaCount: 0` | ❌ HPA не опускается ниже `minReplicas` |
| Внешние метрики | ✅ триггеры в ScaledObject | ⚠️ PromQL-запросы нужно мапить в custom-metrics через ConfigMap |
| Готовые скейлеры | ✅ 60+ скейлеров (Kafka, RabbitMQ, Redis и т. д.) | ❌ только то, что сам опишешь правилами |
| Смена триггера | ✅ меняется один блок в ScaledObject | ⚠️ правка правил adapter + перезапуск пода |
...
xychart-beta
    ...
    x-axis ["t0","t1","t2","t3","t4","t5","t6","t7","t8","t9","t10","t11","t12","t13","t14","t15","t16","t17","t18","t19","t20","t21","t22","t23","t24"]
    ...
    line "Node overprovisioning" [53,76,94,100,94,76,53,29,11,5,11,29,53,76,94,100,94,76,53,29,11,5,11,29,53]
```
...
| Параметр | Значение |
|---|---|
| Статические ноды | 100% |
| Node overprovisioning (среднее) | **~52.5%** |
| Экономия | **~47.5%** |
