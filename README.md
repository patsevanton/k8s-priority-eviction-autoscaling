...
Задача, знакомая многим командам: в кластере Kubernetes живут критически важные сервисы, и когда приходит пик нагрузки, обычно выбирают между двумя плохими вариантами: держать статический пул нод, которые ночью простаивают и стоят денег, или положиться на cluster autoscaling — и тогда бизнес-поды проведут в статусе Pending до 10 минут, пока автоскейлер развернёт новую ноду.
...
Получается замкнутый цикл: **запас placeholder-подов занят на нодах → приходит пик → placeholder вытесняется за секунды → реальный под запущен → placeholder в Pending → новая нода → буфер восстановлен**. Кластер переживает пиковые нагрузки без ручного вмешательства и без ожиданий до 10 минут.
