...
│ свободные ресурсы:                    │   │ HPA создаёт                           │   │ capacity-overprovisioning поды        │   │ CA создаёт ноду                       │   │ capacity-overprovisioning под Running │
│ capacity-overprovisioning под         │   │ новые реплики                         │   │ переходят в                           │   │                                       │   │ буфер готов                           │
...
Нагрузка/HPA                            Планировщик                                            Cluster Autoscaler
   ...
   │ новая реплика           │                                         │
   │─────────────────────►   │                                         │
   │                         │ capacity-overprovisioning под имеет     │
   │                         │ приоритет -1000                         │
   │                         │ → можно вытеснить                       │
   │                         │                                         │
   │                         │ вытеснение:                             │
   │                         │ capacity-overprovisioning под → Pending │
   │                         │─────────────────────────────►           │
   │                         │                                         │ получает сигнал
   │                         │                                         │
   │                         │                                         │ создаёт ноду
   │                         │                                         │
   │                         │ буфер восстановлен:                     │
   │                         │ capacity-overprovisioning под Running   │
...
│ capacity-overprovisioning под держит буфер  │
└─────────────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────┐
...
│                                                                 ▲
┌─────────────────────────────────────────────────────────────┐   │
│ Дежурный режим: capacity-overprovisioning под держит буфер  │   │
└─────────────────────────────────────────────────────────────┘   │
                               │                                  │
                               ▼                                  │
┌─────────────────────────────────────────────────────────────┐   │
  ...
  template:
    ...
    spec:
      ...
      - name: pause
        ...
        resources:
          requests:
            cpu: 900m # Размер резерва, который держит один capacity-overprovisioning под
            memory: 1Gi
```
Ключевые детали:
