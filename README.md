...
└──────────────────────────────────────────────────┘
                         ...
                         ▼
┌──────────────────────────────────────────────────┐
│ KEDA увеличивает количество подов business-app   │
└──────────────────────────────────────────────────┘
                         │
                         ▼
┌──────────────────────────────────────────────────┐
│ business-app под в Running                       │
│ capacity-overprovisioning под в Pending          │
└──────────────────────────────────────────────────┘
                         │
                         ▼
┌──────────────────────────────────────────────────┐
│ Cluster Autoscaler добавляет новую ноду          │
└──────────────────────────────────────────────────┘
                         │
                         ▼
┌──────────────────────────────────────────────────┐
│ capacity-overprovisioning под снова Running      │
│ на новой ноде                                    │
└──────────────────────────────────────────────────┘
```

## Зачем нужен резерв: скорость реакции и защита от троттлинга

Паттерн решает не только вопрос скорости (место под всплеск освобождается за секунды, а не за минуты провижининга новой ноды облаком), но и вопрос плотной упаковки нод.

Если нода плотно заселена подами — сумма `resources.requests` близка к allocatable ноды — у неё не остаётся запаса по зарезервированным ресурсам. При всплеске по CPU поды начинают конкурировать за реальные ресурсы: контейнеры упираются в CPU-троттлинг и замедляются — для приложений, чувствительных к задержкам, это полноценный инцидент.

Capacity-overprovisioning поды «занимают» часть requests ноды, почти не потребляя ресурсов: контейнер `pause` просто спит. Благодаря этому планировщик не может утрамбовать на ноду подов больше, чем позволяет оставшийся объём requests, — нода держит запас. Когда приходит всплеск, бизнес-под с корректными requests вытесняет capacity-overprovisioning под (приоритет -10) и получает зарезервированное место сразу, в пределах своих requests, — а не делит ресурсы с соседями на грани троттлинга.

Резерв защищает ровно в границах requests: если бизнес-под запрашивает меньше, чем фактически потребляет, троттлинг остаётся возможным. Это уже вопрос корректности requests/limits самого приложения, а не паттерна.
...
Обратная сторона: если capacity-overprovisioning поды занимает ноды, а автоскейлер по какой-то причине не может расширить кластер (квота, лимит max, недоступность зон), вытесненные capacity-overprovisioning поды будут ждать в Pending неограниченно долго, и буфер не восстановится. Приоритеты — не замена мониторингу: алерт на долго живущие Pending-поды и на не расширяющиеся node groups должен быть в любом случае.
