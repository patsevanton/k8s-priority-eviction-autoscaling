...
В этой статье разберём, как это устроено внутри, и развернём полный демо-стенд: от KEDA-автоскейлинга бизнес-приложения по RPS, который генерирует генератор нагрузки, до живого вытеснения capacity-overprovisioning подов, которое можно наблюдать своими глазами в `kubectl get events`.
...
Получается замкнутый цикл: **запас capacity-overprovisioning подов занят на нодах → нагрузка растёт → реплики бизнес-приложений увеличиваются (например, через HPA) → capacity-overprovisioning под вытесняется за секунды → реальный под запущен → capacity-overprovisioning под в Pending → новая нода**.
