...
### Шаг 2. Запускаем критичное приложение — имитация роста нагрузки

В реальной жизни новые реплики бизнес-приложений появляются не вручную: нагрузка растёт, HPA увеличивает `replicas`, и новые поды должны запускаться немедленно. В демо мы имитируем этот момент одним `kubectl apply`: critical-app — это «новая реплика, которую HPA только что создал при росте нагрузки».
...
И placeholder запускается уже на ней, восстанавливая резерв. Полный цикл замкнулся за пару минут без единого ручного действия: **рост нагрузки → мгновенное вытеснение → реальный под работает → новая нода → буфер восстановлен**.
