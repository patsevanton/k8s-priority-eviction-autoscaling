...
имитируем этот рост одним `kubectl apply` — critical-app играет роль «реплики,
которую HPA только что создал».
...
Полный цикл замкнулся: **рост нагрузки → мгновенное вытеснение → реальный под
работает → новая нода → буфер восстановлен**.
