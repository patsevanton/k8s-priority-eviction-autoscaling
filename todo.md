...
- `manifests/keda/business-app.yaml` — Deployment + Service бизнес-приложения (requests 900m/1Gi — как у placeholder, чтобы реплика гарантированно вытесняла его).
...
Ожидаемый цикл: рост RPS по лестнице → KEDA поднимает реплики (каждая с requests 900m/1Gi) → новые поды вытесняют placeholder'ы (приоритет -1000) → вытесненные placeholder'ы в Pending → Cluster Autoscaler добавляет ноду → буфер восстанавливается. При спаде RPS до 0 (ночь) KEDA возвращёт реплики к 1, CA удаляет лишние ноды.
