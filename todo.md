...
- [ ] Проверить генерацию RPS по закону Симпсона на живом кластере: RPS должен плавно расти 0→1000 по S-образной кривой (излом в точке моды 600), после `CYCLE` держаться на 1000, KEDA при этом поднимает реплики business-app. Образ `load-generator` должен быть собран и запушен в GHCR.
